...
function poinToSVGLine({ x, y }) {
    return `${x},${y} `;
}
function polygonToSVGPath(points) {
    return points.map(poinToSVGLine).join(' ');
}
function scoresToSVGPolygon(scores, polygonWidth = 200) {
    return polygonToSVGPath(new _score_polygon__WEBPACK_IMPORTED_MODULE_1__["ScorePolygon"]({ width: polygonWidth, scores }).points);
}
...
let ScorePolygonModule = class ScorePolygonModule {
};
...
function scaleScore(rawScore, maxScore = 10, scale = 10) {
...
let ScorePolygonComponent = class ScorePolygonComponent {
    ...
    ngOnChanges() {
        ...
        const polygon = new _score_polygon__WEBPACK_IMPORTED_MODULE_2__["ScorePolygon"]({
            width: 200,
            scores: rawScores
        });
        const wrapperPolygon = new _score_polygon__WEBPACK_IMPORTED_MODULE_2__["ScorePolygon"]({
            width: 200,
        ...
        this.vortex = Object(_SVG_mappers__WEBPACK_IMPORTED_MODULE_3__["scoresToSVGPolygon"])(rawScores.map(sc => 10), 200);
...
ScorePolygonComponent.ctorParameters = () => [
...
let AppComponent = class AppComponent {
    ...
    updateEdges(quantity) {
        this.scores = [..._assets_data_scores__WEBPACK_IMPORTED_MODULE_3__["SCORESMOCK"]].splice(0, quantity);
        this.randomizeScores();
    }
    randomizeScores() {
        this.scores = this.scores.map(score => (Object.assign({}, score, { score: (Math.random() * 10).toFixed(0) })));
    }
    updateColor(color) {
